<?php
/*
Plugin Name: NewProBot - TeploBot addon for NewPro project.
Description: Addon with custom directives for TeploBot plugin for the NewPro website.
Version: 0.1
Author: Teplitsa
...
Domain Path: /lang
Contributors:
    Anna Ladoshkina aka foralien (webdev@foralien.com) - UX, Idea
	Lev Zvyagintsev aka ahaenor (ahaenor@gmail.com) - Development
...
// Plugin version:
if( !defined('NPRTB_VERSION') )
    define('NPRTB_VERSION', '0.1');
	
// Plugin DIR, with trailing slash:
if( !defined('NPRTB_PLUGIN_DIR') )
    define('NPRTB_PLUGIN_DIR', plugin_dir_path( __FILE__ ));

// Plugin URL:
if( !defined('NPRTB_PLUGIN_BASE_URL') )
    define('NPRTB_PLUGIN_BASE_URL', plugin_dir_url(__FILE__));
	
// Plugin ID:
if( !defined('NPRTB_PLUGIN_BASE_NAME') )
    define('NPRTB_PLUGIN_BASE_NAME', plugin_basename(__FILE__));
...
if( !defined('PHP_VERSION') || version_compare(PHP_VERSION, '5.3.0', '<') ) {
    ...
    echo '<div id="message" class="error"><p><strong>Внимание:</strong> версия PHP ниже <strong>5.3.0</strong>. Плагин бота нуждается в PHP хотя бы <strong>версии 5.3.0</strong>, чтобы работать корректно. Плагин будет деактивирован.<br /><br />Пожалуйста, направьте вашему хостинг-провайдеру запрос на повышение версии PHP для этого сайта.</p> <p><strong>Warning:</strong> your PHP version is below <strong>5.3.0</strong>. Bot plugin needs PHP <strong>v5.3.0</strong> or later to work. Plugin will be deactivated.<br /><br />Please contact your hosting provider to upgrade your PHP version.</p></div>';

    die();
}	
	
load_plugin_textdomain('nprtb', false, '/'.basename(NPRTB_PLUGIN_DIR).'/lang');
